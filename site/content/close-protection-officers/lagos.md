---
title: "Close Protection Officers in Lagos"
description: "NSCDC-licensed close protection officers in Lagos, Nigeria. CPO cover for executive visitors, energy sector principals, and HNWI clients in Victoria Island and Lekki."
slug: "lagos"
h1: "Close Protection Officers in Lagos, Nigeria"
seo_title: "Close Protection Officers Lagos | NSCDC CPO Nigeria"
service: "close-protection-officers"
city: "Lagos"
country: "Nigeria"
risk_level: "critical"
hero_image: "city-lagos-hero.webp"
date: "2026-06-03"
cta_text: "Planning operations in Lagos? Request NSCDC-licensed CPO cover for your delegation."
components:
  - title: "NSCDC-Licensed CPO Operators"
    description: "Commercial close protection in Nigeria operates under Nigeria Security and Civil Defence Corps (NSCDC) licensing. Operating companies and their CPOs must hold NSCDC authorisation. We engage only NSCDC-licensed local operators with verifiable credentials."
  - title: "Targeted Abduction Protocols"
    description: "Lagos CPO work is directly shaped by the documented risk of targeted abduction affecting senior executives and HNWI individuals. Operational planning includes information control around principal movements, route variation, counter-surveillance awareness, and pre-determined emergency extraction contacts."
  - title: "Victoria Island, Ikoyi, and Lekki Coverage"
    description: "Operational familiarity with the corporate and residential zones most international executives use: Victoria Island, Ikoyi, and Lekki Phase 1. Pre-advance work at hotels, offices, and major meeting venues in these zones is standard for any multi-day Lagos engagement."
  - title: "Murtala Muhammed Airport Protocols"
    description: "LOS arrival and departure are the two highest-risk windows for foreign visitors in Lagos. CPO cover begins before customs exit at arrivals and extends to airside drop-off at departure. The LOS-to-Victoria-Island corridor is a documented high-incident route."
  - title: "Armed CPO Options Where Appropriate"
    description: "Armed close protection is available through NSCDC-licensed operators for principals in sectors with the highest targeting profiles: energy, financial services, and senior government-adjacent roles. Armed options are specified based on the threat assessment, not as a default."
  - title: "Two-Person Minimum Team Configuration"
    description: "Lagos CPO engagements typically operate as a minimum two-person team: a CPO and an NSCDC-licensed security driver. The threat environment and kidnap risk vectors make a lone CPO configuration unsuitable for senior principals in this market."
faqs:
  - question: "What licence does a close protection officer need in Lagos?"
    answer: "Commercial close protection in Nigeria requires licensing from the Nigeria Security and Civil Defence Corps (NSCDC) under the NSCDC Act 2003. The operating company must hold an NSCDC licence, and individual CPOs operate under that company's authorisation. Verify the NSCDC licence number of any operating company proposed for your Lagos detail. A company that cannot provide this number on request is not in compliance."
  - question: "How high is the kidnap risk for senior executives in Lagos?"
    answer: "The FCDO and US State Department both maintain travel advisories noting kidnap risk in Nigeria, including Lagos. The risk concentrates on principals with publicly visible wealth or seniority in the energy or financial sectors, particularly during airport arrivals and vehicle movements. The operational response is structured ground movement with CPO cover, security drivers, and information control around the principal's itinerary."
  - question: "What is the standard CPO team size in Lagos?"
    answer: "For senior corporate principals in Lagos, the standard CPO configuration is a minimum two-person team: a CPO and a trained security driver, with operations controller support. For higher-profile engagements, a three-person team adding a second CPO or dedicated advance officer is used. Solo CPO cover is not recommended in Lagos given the vehicle-based threat environment and the need for separate driver and protection functions."
  - question: "How much does CPO cover cost in Lagos?"
    answer: "NSCDC-licensed CPO cover in Lagos ranges from approximately USD 400 to USD 900 per officer per day for a vetted, locally licensed CPO as at June 2026. A two-person team with security driver is typically USD 700 to USD 1,400 per day. Armed options and armoured vehicle integration are priced separately based on the operational requirement and duration."
---

This page is about the people who provide close protection in Lagos: who they are, how they are vetted, and what credentials to verify before they deploy. Every operator in our Lagos network holds current NSCDC registration, passes documented background checks, and is assessed for Lagos-specific competence. It covers standard team configuration, the two-person minimum for higher-risk principals, armed CPO options under NSCDC licensing, and the checks a buyer should insist on before engaging any officer.

## NSCDC licensing and compliance

Nigeria's commercial security sector, including close protection, operates under the Nigeria Security and Civil Defence Corps (NSCDC) Act 2003. The NSCDC issues operating licences to companies providing commercial security services; individual CPOs operate under the employing company's authorisation. Foreign security companies cannot operate in Nigeria without NSCDC-registered local partners.

The practical verification step is to confirm the NSCDC licence number of the operating company proposing your Lagos detail. A company that cannot provide this on request is not compliant.

## The Lagos CPO operating environment

The threat environment shaping CPO work in Lagos involves vehicle-based crime, targeted abduction, and the specific risk of the airport-to-accommodation transfer. Kidnap-for-ransom targeting of senior business executives is documented in NSCDC and security sector incident reporting. The operational response combines qualified CPO cover, NSCDC-licensed security drivers, information control around the principal's movements, and operations controller oversight throughout.

For complementary services in Lagos, see our [Lagos city page](/cities/lagos/) and [security drivers in Lagos](/security-drivers/lagos/).

To arrange short-term cover for a specific trip, see [bodyguard hire in Lagos](/bodyguard-hire/lagos/). For organisational, duty-of-care protection around a corporate visit, see [executive protection in Lagos](/executive-protection/lagos/).
